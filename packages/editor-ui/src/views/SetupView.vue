...
<template>
  <div class="auth-screen">
    <form class="auth-form" @submit.prevent="onSubmit">
      ...
      <label>
        Password
        <RunnelInput v-model="password" type="password" placeholder="At least 8 characters" />
      </label>
      <p v-if="error" class="auth-error">{{ error }}</p>
      <RunnelButton :disabled="submitting">{{ submitting ? 'Creating…' : 'Create account' }}</RunnelButton>
    </form>
  </div>
</template>
